...
def _load_market_from_env() -> dict[str, Any]:
    raw = os.environ.get("MARKET_CONFIG_JSON")
    if not raw:
        return {}
    try:
...
def update_dashboard_section(
    section: Literal["product", "table", "cards", "positioning", "market"],
    ...
    `section`:
      ...
      - 'market'       → replaces state['market'] (MarketConfig snapshot)
    """
