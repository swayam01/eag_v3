...
def _competitor_item(c: dict[str, Any]) -> None:
    ...
    fit_score = c.get("market_fit_score", 0)
    fit_chip = f"  ·  fit {fit_score}/10" if fit_score else ""
    suffix = f"  ·  {hq}" if hq else ""
    title = f"{name}  ·  {_short(pricing, 50)}{suffix}{fit_chip}"
    with AccordionItem(title):
        with Column(gap=4, css_class="pt-2"):
            Text(c.get("description", ""), css_class="text-sm text-slate-700")
            if c.get("market_fit_reason"):
                with Row(gap=2, css_class="items-center"):
                    Badge(f"Market fit {fit_score}/10", variant=_fit_variant(fit_score))
                    Text(c["market_fit_reason"], css_class="text-sm text-slate-700")
...
def _fit_variant(score: Any) -> str:
    try:
        s = int(score or 0)
