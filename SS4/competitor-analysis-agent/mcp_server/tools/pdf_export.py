...
def export_dashboard_pdf(out_path: str | None = None) -> str:
    ...
    table_data = [["Name", "HQ", "Pricing", "Top features", "Target market"]]
    for c in competitors:
        table_data.append(
            [
                c.get("name", "—"),
                _short(c.get("headquarters") or "—", 25),
                _short(c.get("pricing", "—"), 40),
                _short(", ".join(c.get("features", [])[:4]), 60),
                _short(c.get("target_market", "—"), 40),
            ]
        )
    if len(table_data) > 1:
        tbl = Table(
            table_data,
            repeatRows=1,
            colWidths=[1.3 * inch, 1.1 * inch, 1.3 * inch, 2.4 * inch, 1.3 * inch],
        )
        tbl.setStyle(
            TableStyle(
                [
